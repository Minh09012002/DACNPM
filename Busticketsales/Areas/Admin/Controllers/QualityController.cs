...
namespace Busticketsales.Areas.Admin.Controllers
{
    ...
    public class QualityController : Controller
    {
        ...
        {
            var contact = _context.ContactQualities.Where(m => (m.IsActive == true) && (m.Position == 2)).OrderBy(m => m.ContactQualityID).ToList();
            return View(contact);
        }
        ...
        public IActionResult Create(ContactQuality quality)
        {
            if (ModelState.IsValid)
            {
                _context.ContactQualities.Add(quality);
        ...
        public IActionResult Details(long? id)
        {
            ...
            var quality = _context.ContactQualities
        ...
        public IActionResult Delete(long id)
        {
            var Deleteq = _context.ContactQualities.Find(id);
            if (Deleteq == null)
            {
                return NotFound();
            }
            _context.ContactQualities.Remove(Deleteq);
            _context.SaveChanges();
            return RedirectToAction("Index");
        }
    }
